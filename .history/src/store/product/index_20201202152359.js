...
export default {
  ...
  actions: {
    investProduct({commit},payload){
      commit('setLoading',true)
      // const user = getters.user.id;
      // const users = getters.loadUsers;
      // console.log(user);
      // console.log(users);
      const registeredProducts = {registeredProducts:payload};
      ...
      .then(data => {
        ...
        for (let key in obj) {
          profiles.push({
            ...
            registeredProducts:obj[key].registeredProducts
          });
        }
        console.log(profiles)

        function locateUser(current){
          return current.userid === b.user.user.id
        }
        const locatedUser = profiles.find(locateUser).id
      ...
      // firebase.database().
      // ref('/profiles/').
      // child('-M8swOCMkSWIP7UCwSIG').
      // update(registeredProducts).
      // then(()=>{
      //   commit('setLoading',false)
