...
export default {
  ...
  mutations: {
    ...
    registerUserForProduct(state,payload){
      const id = payload.id
      if(state.user.loadedProducts.findIndex(product=>product.id === id)>=0){
        return 
      }
      state.user.loadedProducts.push(id)
  ...
  actions: {
    investProduct({commit,getters},payload){
      ...
      firebase.database().ref('/profiles' + user.id).child('/registration/')
      .push(payload)
      .then((data)=>{
