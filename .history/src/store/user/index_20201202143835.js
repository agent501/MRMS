...
export default {
  ...
  mutations: {
    ...
    updateUserBalance(state, payload) {
      const product = state.loadUsers.find((product) => {
        return product.id === payload.id;
      });
      if (payload.currentbalance) {
        product.currentbalance = payload.currentbalance;
      }
    },
  },
  actions: {
    loadUsers({ commit }) {
      ...
      .then(data => {
        ...
        for (let key in obj) {
          ...
          commit('setLoadedUsers', profiles);
    ...
    autosignin({ commit }, payload) {
      commit('setUser', { id: payload.uid, registeredProducts: [],fbKeys:{} });
    },
    logout({ commit }) {
    ...
    updateuserbalance({ commit }, payload) {
      ...
      const updateObj = {};
      if (payload.currentbalance) {
        updateObj.currentbalance = payload.currentbalance;
      }
      firebase
        .database()
        .ref('profiles')
