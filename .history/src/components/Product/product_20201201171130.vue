    ...
    <v-row no-gutters>
      ...
      <v-col cols="12" md="6">
        ...
        <v-card class="text-capitalize" outlined>
             <!-- register product -->
            <app-register-product :product1="product.id" :product2="product.investmentperunit" style="margin-top:10px;margin-left:10px"></app-register-product>  

          <v-card-title class="text-uppercase font-weight-bold display-1">
            Product details {{getuser.name}}
          ...
          <v-list-item>
            <v-list-item-content>total investment amount</v-list-item-content>
            <v-list-item-content
              class="subtitle-1 font-weight-thin"
            >: RM{{ product.investmentperunit * 5 }}</v-list-item-content>
          </v-list-item>
...
export default {
  ...
  computed: {
    getuser() {
      return this.$store.getters.loadedUsers(this.test);
    },
    product() {
